...
/*   Updated: 2025/08/11 15:27:58 by mpierce          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef BONUS_MINIRT_H
# define BONUS_MINIRT_H
...
t_color			stripe_at_object(t_pattern pattern, t_object obj,
					t_tuple world_point);
/* -------------------------------------------------------- minirt_color07.c */
t_color			gradient_at(t_pattern gradient, t_tuple point);
t_color			gradient_at_object(t_pattern pattern, t_object obj,
					t_tuple world_point);
/* -------------------------------------------------------- minirt_color08.c */
t_color			ring_at(t_pattern ring, t_tuple p);
t_color			ring_at_object(t_pattern pattern, t_object obj,
					t_tuple world_point);
/* -------------------------------------------------------- minirt_color09.c */
t_color			checker_at(t_pattern check, t_tuple p);
t_color			checker_at_object(t_pattern pattern, t_object obj,
					t_tuple world_point);
/* -------------------------------------------------------- minirt_color10.c */
...
/* ---------------------------------------------------------- minirt_ray08.c */
void			truncate_cylinder(t_object *cylinder, t_ray ray,
					t_intersections *res);
void			truncate_cone(t_object *cone, t_ray ray,
					t_intersections *res);
/* ---------------------------------------------------------- minirt_ray09.c */
...
/* ---------------------------------------------------------- minirt_ray20.c */
t_color			lighting(t_lighting_param p, t_material m, t_light light,
					t_tuple v[3]);
bool			is_shadowed(t_world world, t_tuple point);
/* ---------------------------------------------------------- minirt_ray21.c */
t_computations	prepare_computations(t_world w, t_intersection *i, t_ray r);
/* ---------------------------------------------------------- minirt_ray22.c */
void			prepare_refraction_calculations(t_world *w,
					t_computations *comps, t_intersection *target);
/* ---------------------------------------------------------- minirt_ray23.c */
...
/* ------------------------------------------------------- minirt_object02.c */
t_object		cube(t_tuple origin, t_tuple direction, t_color col,
					t_float cb_scale);
/* ------------------------------------------------------- minirt_object03.c */
...
bool			is_inside_container(t_obj_container *container, t_object *obj,
					int *index);
...
void			data_null_check(t_minirt *rt, char **data, char **tmp,
					char *line);
...
void			cylinder_check(t_minirt *rt, char **origin, char **vec,
					char **rgb);
