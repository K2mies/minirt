...
/*   Updated: 2025/07/09 16:23:24 by rhvidste         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
/**
...
t_intersections	intersect_caps(t_object *cylinder, t_ray ray, t_intersections *xs)
{
	...
	if (cylinder->closed == false || compare_floats(ray.direction.y, 0.0f))
		return (res);
	res.t[2] = (cylinder->min - ray.origin.y) / ray.direction.y;
	if (check_cap(ray, res.t[2], cylinder))
		res.count += 1;
	res.t[3] = (cylinder->max - ray.origin.y) / ray.direction.y;
	if (check_cap(ray, res.t[3], cylinder) )
		res.count += 1;
	return (res);
}
